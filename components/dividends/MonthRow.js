      ...
      <div className={styles.monthFlexContainer}>
        ...
        <div className={styles.tableContainer}>
            ...
            .map((div) => (
              <div className={styles.divFlexContainer} key={uuidv4()}>
                <h6 className={styles.ticker}>
                  <Link
                    href={`/portfolio/[ticker]?ticker=${div.ticker}`}
                    as={`/portfolio/${div.ticker}`}
                  >
                    <a className={styles.tickerAnchor}>{div.ticker}</a>
                  </Link>
                </h6>
                ...
                <div className={styles.cellRed} title="shares * cost basis">{div.netCostBasis || ""}</div>
                <div className={styles.cellBlue} title="amount * frequency(i.e. quarterly = 4) / cost basis">
                  {div.yield ? `${div.yield}%` : ""}
                </div>
          ...
          <div className={styles.summaryContainer}>
            ...
            <div className={styles.cellRed} title="sum of net cost bases">${netCostBasisTotal}</div>
            <div className={styles.cellBlue} title="sum of yields / # of yields">{avgYield ? `${avgYield}%` : ''}</div>
